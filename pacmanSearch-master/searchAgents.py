...
class PositionSearchProblem(search.SearchProblem):
    ...
    def __init__(self, gameState, costFn = lambda x: 1, goal=(1,1), start=None, warn=True, visualize=True):
        ...
        goal: A position in the gameStat
        """
...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        if self.startingPosition == self.corners[0]:
            ...
            temp[0] = True
            self.cornersReached = tuple(temp)
        ...
        if self.startingPosition == self.corners[3]:
            ...
            temp[3] = True
            self.cornersReached = tuple(temp)
        self.startState = (self.startingPosition, self.cornersReached)
        ...
        for direction in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                ...
                if (nextx, nexty) == self.corners[3]:
                    nextCorners[3] = True
                successors.append( ( ((nextx, nexty), tuple(nextCorners)), direction, 1) )
        return successors
...
def cornersHeuristic(state, problem):
    ...
    while i < 4:
        ...
        i += 1
    for j in unvisited:
        dist = max(dist, util.manhattanDistance(state[0], corners[j]))
    return dist
